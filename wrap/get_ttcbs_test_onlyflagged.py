...
onlyflag=True    # To run on file which shows only flagged cloud bands
...
for d in range(ndset):
    ...
    for m in range(nmod):
        ...
        if onlyflag:
            indir=bkdir+"somsfromc/olrdata_ttcbdays/"
            if testyear: indir = indir + 'test/'
        else:
            indir=bkdir+"metbot_multi_dset/"+dset+"/"
            outdir = indir + name + "/"
            if testyear: outdir = outdir + 'test/'
            my.mkdir_p(outdir)
            outsuf = outdir + name + '_'
        if onlyflag:
            infile=indir+dset+"."+name+"."+globv+".day.mean.ttcbdays.thresh_actual.nc"
        else:
        ...
        if calcthresh:
            ...
            threshtxt=bkdir+'metbot_multi_dset/thresholds.fmin.all_dset.txt'
            print threshtxt
        ...
        for t in range(nthresh):
            ...
            if addrain:
                ...
                rainfile = bkdir + 'metbot_multi_dset/'+ raindset + "/" + rainmod + ".pr.day.mean." + ys + ".nc"
                print rainfile
