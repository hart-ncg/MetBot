...
def gridrainmap_season(s,eventkeys,rain,rlat,rlon,rdtime,cl,season='coreseason',key='noaa-olr-0-0',\
    ...
    for mn in mns:
        ...
        if labels:
            if ptype=='tot_ttt':
                tit=stats.mndict[mn]+': '+str(nttt_mon)+' TTT days '+str(float(nttt_mon)/float(nys))+'/yr'
            elif ptype=='per_ttt':
                tit=stats.mndict[mn]+': '+str((float(nttt_mon)/float(ndays_mon))*100.0)+'% of days have TTTs'
        else:
            tit=stats.mndict[mn]
        plt.title(tit)
